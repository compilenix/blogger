...
class Helper {
	 ...
	 * @returns {string[] | Object[]} string[] if includeUnpublished = false else Object[] => { id, isPublished }
	 * @memberOf Helper
	 */
	static getAllPostIds(reverseOrder, includeUnpublished = false) {
		...
		for (let i = 0; i < posts.length; i++) {
			...
			let isPublished = fs.existsSync(config.post.DirectoryPosts + this.GetFsDelimiter() + posts[i] + ".asc");
	...
	static getTitles(reverseOrder, includeUnpublished = false) {
		const data = [];
		let list = this.getAllPostIds(reverseOrder, includeUnpublished);
