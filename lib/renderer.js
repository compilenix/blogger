...
class Renderer {
	constructor() {
		...
		this.fields.path.staticContentUri = global.Config.staticContentUri;
		this.fields.title = global.Config.title;
		this.fields.author = global.Config.Author;
	}
	...
	includeTemplatesRecursive(template, degreeOfRecursion = 0) {
		var regex = /\[\[([A-Za-z0-9.]+)\]\]/g;
		var match = regex.exec(template);

		if (match !== null && degreeOfRecursion < 10) {
			return this.includeTemplatesRecursive(this.includeTemplates(template), degreeOfRecursion + 1);
		} else {
			return template;
		}
	}
	...
	render (template) {
		return require("mustache").render(this.includeTemplatesRecursive(template), this.fields);
	}
}
