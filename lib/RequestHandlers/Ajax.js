...
class Ajax {
  ...
  processPost (ajax, data, request) {
    ...
    switch (post.action) {
      ...
      case 'togglePublish':
      {
        log.debug('Ajax post is a "togglePublish"')
        let succeded = false
        const result = ajax.togglePublish(post.postid)
        ...
        dataToReturn = `{ "OperationSucceded": ${succeded} }`
        ajax.response.code = 200

        log.debug(`Ajax post action result of "togglePublish" is: ${JSON.stringify(result)} -> ${dataToReturn}`)
        break
      }
      default:
  ...
  postList () {
    return JSON.stringify({
      type: 'postlist',
      list: Helper.getAllPostIds(/* reverseOrder */ true, /* includeUnpublished */ true),
      titles: Helper.getTitles(/* reverseOrder */ true, /* includeUnpublished */ true)
    })
  }
  ...
  togglePublish (postid) {
    if (postid) {
      return Helper.TogglePublish(postid)
    }
  ...
  previewPost (post) {
    if (post.content) {
      return Helper.getPage(`<article>${JSON.parse(post.content) || post.content || ''}</article>`, post.title || undefined)
    }
  }
