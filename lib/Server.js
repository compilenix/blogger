...
class Server {
    ...
    _sendData(data, request, response, writeCache) {
        ...
        if (data.type === "file" && data.content) {
            let filePath = path.normalize(`${__dirname}${Helper.GetFsDelimiter()}..${Helper.GetFsDelimiter()}${data.content}`);
            ...
            let lastModified = new Date(fs.statSync(data.content).mtime);
            response.setLastModified(lastModified);
