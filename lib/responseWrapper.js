...
_responseWrapper.prototype.addHttp2PushEntity = function (path, data, httpCode, header) {
	this.push.push({
		path: path,
		data: data,
		httpCode: httpCode,
...
_responseWrapper.prototype.send = function () {
	if (global.Config.EnableHttp2 && global.Config.Https.Enabled && this.response.push) {
		this.push.forEach(function(element) {
			var currentPush = this.response.push(element.path);
