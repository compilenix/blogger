...
Helper = require(".." + GetFsDelimiter() + "helper.js");
...
tests.push({
	test: function() {
		...
		pass &= test_equal(response.getContent(), dummy_content, "content as expected after setting");
		pass &= test_equal(response.contentLength, 26, "content length as expected after setting");

		response.setResponseCode(200);
		pass &= test_equal(response.getResponseCode(), 200, "response code as expected after setting");

		response.setContentType(dummy_mimetype);
		pass &= test_equal(response.getContentType(), dummy_mimetype, "content type as expected after setting");
...
tests.push({
	test: function() {
		...
		pass &= test_true(cache.has(dummy_request), "cache file present after adding");
		cache.clear();
		pass &= test_false(cache.has(dummy_request), "cache file not present after cache reap");
		cache.add(dummy_request, dummy_content, dummy_mimetype, dummy_mimetype);
		pass &= test_true(cache.has(dummy_request), "cache file present after re adding");
		cache.del(dummy_request);
		pass &= test_false(cache.has(dummy_request), "cache file not present after delete");
...
tests.push({
	test: function() {
		...
		cache.add(dummy_request, dummy_content, dummy_mimetype, dummy_mimetype);
		pass &= test_true(cache.has(dummy_request), "mem cache entry present after adding");
		cache.clear();
		pass &= test_false(cache.has(dummy_request), "mem cache entry not present after cache reap");
		cache.add(dummy_request, dummy_content, dummy_mimetype, dummy_mimetype);
		pass &= test_true(cache.has(dummy_request), "mem cache entry present after re adding");
		cache.del(dummy_request);
		pass &= test_false(cache.has(dummy_request), "mem cache entry not present after delete");
		const dummy_response = new ResponseWrapper();
		dummy_response.send = () => { };
...
tests.push({
	test: function() {
		...
		let helper = require(".." + GetFsDelimiter() + "Helper.js");

		let data = helper.getPosts(false);
...
tests.push({
	test: function() {
		...
		// TODO fix RequestHandler stuff
		return false;

		let pass = true;

		let find = require(".." + GetFsDelimiter() + "requestHandlers" + GetFsDelimiter() + "Find.js");
		const responseWrapper = require(".." + GetFsDelimiter() + "responseWrapper.js").ResponseWrapper;
		Cache = true;
		const dummy_response = new responseWrapper();
		dummy_response.send = function () { };

		let dummy_request = { url: "/find/" };
		let data = find.Find(dummy_request);
