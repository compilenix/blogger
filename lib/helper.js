...
function prepareHeaderHtml(title, header) {
    ...
    if (title) {
        header = replaceAll("{{Title}}", htmlenclode(Config.Title + " - " + title), header);
    } else {
        header = replaceAll("{{Title}}", htmlenclode(Config.Title), header);
    }

    header = replaceAll("{{Author}}", htmlenclode(Config.author), header);
