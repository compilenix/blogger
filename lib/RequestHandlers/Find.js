...
var FileHeader = global.Config.post.FileHeader || "header.html";
var FileFooter = global.Config.post.FileFooter || "footer.html";
...
function Find(request) {
	...
	const posts = global.Helper.getPosts(false);

	for (let index = 0; index < posts.length; index++) {
		let postId = posts[index];
		postsData.push(global.Helper.getPost(posts[index]));
		postIds.push(postId);
	}
	...
	for (let index = 0; index < postsData.length; index++) {
		...
		if (match) {
			deps.push(DirectoryPosts + global.Helper.GetFsDelimiter() + postId + ".json");
			matched.push(index);
			continue;
		}
	...
	return {
        type: "content",
        code: 200,
        content: global.Helper.getPage(content, "Finding: " + query),
        mimetype: "text/html"
    };
}
