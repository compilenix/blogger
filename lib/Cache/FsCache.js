...
class FsCache extends NullCache {
	...
	del(req) {
		if (this.has(req)) {
			...
			log.info(`Remove cache file: ${file}`);
			fs.unlinkSync(file);
		}
	}
	...
	clear() {
		...
		for (let i = 0; i < cacheFileList.length; i++) {
			log.info("Remove cache file: " + this.DirectoryCache + Helper.GetFsDelimiter() + cacheFileList[i]);
	...
	add(req, cont, mime, code) {
		...
		log.info(`Add cache file: ${this._path(req)}`);
