...
tests.push({
	test: () => {
		...
		cache.add(dummy_request, dummy_content, dummy_mimetype, 200);
		cache.send(dummy_request, dummy_response);
		pass = pass && test_equal(zlib.gunzipSync(new Buffer(dummy_response.data)).toString(), dummy_content, "content_gzip as expected");

		dummy_request = { url: "test/?x=y", headers: { "accept-encoding": "deflate,gzip" } };
		cache.add(dummy_request, dummy_content, dummy_mimetype, 200);
		cache.send(dummy_request, dummy_response);
		pass = pass && test_equal(zlib.unzipSync(new Buffer(dummy_response.data)).toString(), dummy_content, "content_deflate as expected");
