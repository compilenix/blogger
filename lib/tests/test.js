...
config.post.DirectoryPosts = "tests" + GetFsDelimiter() + "posts";
...
var startTest = function(testId) {
    if (testId < testData.length) {
        ...
        config.server.Link = "http://127.0.0.1:" + config.server.port;
        config.post.DirectoryPosts = "tests" + GetFsDelimiter() + "posts";
        config.DirectoryCache = "tests" + GetFsDelimiter() + "cache";
        ...
        node = spawn("node", ["--use_strict", "../app.js", "config", JSON.stringify(testConfig)]);
